...
//	}
    ...
    public static double calculateRentCosts(double noiseImmission, double affectedFacilityUnits, double timeInterval, double timeBinSize) {
        double rentCostsPerFacility = (Math.round(noiseImmission) - 54.0) * 89.50;
        return (rentCostsPerFacility) * affectedFacilityUnits;
    }

    public static double calculateHealthCosts(double noiseImmission, double affectedAgentUnits, double timeInterval, double timeBinSize) {
        double hypertensionCostsPerAgent = (Math.round(noiseImmission) - 56.0) * 16.72;
        double ischemicHeartDiseaseCostsPerAgent = (Math.round(noiseImmission) - 64.0) * 19.14;
        return (hypertensionCostsPerAgent + ischemicHeartDiseaseCostsPerAgent) * affectedAgentUnits;
    }

	public static double calculateDamageCosts(double noiseImmission, double affectedAgentUnits, double affectedFacilityUnits, double timeInterval, double timeBinSize) {

	    double rentCostsPerFacility = (Math.round(noiseImmission) - 54.0) * 89.50;
		double hypertensionCostsPerAgent = (Math.round(noiseImmission) - 56.0) * 16.72;
		double ischemicHeartDiseaseCostsPerAgent = (Math.round(noiseImmission) - 64.0) * 19.14;

		return (hypertensionCostsPerAgent + ischemicHeartDiseaseCostsPerAgent) * affectedAgentUnits + (rentCostsPerFacility) * affectedFacilityUnits;
	}
